<!--
Copyright (c) 2018 Timo Schwarzer (https://github.com/timoschwarzer)
-->
<template>
...
<script>
  ...
  export default {
    ...
    computed: {
      showPipelineIds () {
        return getParameterByName('showPipelineIds') !== null ? !!getParameterByName('showPipelineIds') : true
      },
      showDurations () {
        return (getParameterByName('showDurations') !== null ? !!getParameterByName('showDurations') : true) &&
          (
      ...
      showUsers () {
        return getParameterByName('showUsers') !== null ? !!getParameterByName('showUsers') : false
      },
      durationString () {
    ...
    methods: {
      async fetchJobs () {
        this.$data.jobs = await getJobs(this.$props.project.id, this.$props.pipeline.id)
